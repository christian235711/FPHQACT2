...
create or replace
PACKAGE BODY FPHQACT2 AScreate or replace
PACKAGE BODY FPHQACT2 AS
...
And T90.Py_Cd_Pays = 'ES';  -- AJOUT MARDI
   ...
   BEGIN
        ...
        RES := FPH.FPHQAUT.F_WRITE(FILE_ID, '-------------     BEGIN     ------------------');
				RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'ALIM_DONNEES_FPHQT2_ES ## Alimentation de la table FPHQT_ES_CTT#');  /* T */  /* ALIM_FDSQTRFP et FDSQTRFP avant  */
				RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'ALIM_DONNEES_FPHQT2_ES ## Flux plusieurs tables   ##'); /*  TT  */
		
		
				FOR S_FDHT_ALIM  IN C_FDHT_ALIM  LOOP  /*  TT IN TT  */   /* S_FDSQA_FORMA_CHRISTIAN */
        ...
        --  val_ecart_km := 2 ;
         -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km  =  ' ||val_ecart_km);
 /*CASE CTR_KM_PIVOT WHEN 0 THEN  null
                WHEN NULL then null
                else case GIFP_PROJ_KM_FCT when 0 then  null
                                          when null then null
...
PDV_FOURNISSEUR_VILLE  =  S_FDHT_ALIM.PDV_FOURNISSEUR_VILLE 
  ...
  FUNCTION EXPORT_FPHQT2_ES (NOMLOG      VARCHAR2,
   ...
   -- val_ecart_km  NUMBER;
          ...
          CURSOR C_FDHT_EXP IS  -- TT 

select * from FPH.FPHQT_ES_CTT;  -- AJOUT DU PAYS DEMANDE
        ...
        File_Id_Cvs:= Fph.Fphqaut.F_Open_Cvs(P_Path,File_Name);  -- _UTF8
        ...
        --res := FPH.FPHQAUT.F_WRITE_CVS(file_id_cvs, v_ligne);  -- _UTF8
...
select * from FPH.FPHQT_ES_CTT;  -- AJOUT DU PAYS DEMANDE
        ...
        File_Id := Fph.Fphqaut.F_Open(Nomlog);
        RES := FPH.FPHQAUT.F_WRITE(FILE_ID, '-------------     BEGIN     ------------------');
        Res := Fph.Fphqaut.F_Write(File_Id, 'ALIM_DONNEES_FPHQT2_ES ## DEBUT DE EXPORT FPHQT_ES_CTT#');  /* T */  /* ALIM_FDSQTRFP et FDSQTRFP avant  */
				RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'ALIM_DONNEES_FPHQT2_ES ## NOM DU FICHIER DAT :   ##'||P_FILENAME); /*  TT  */
        ...
        UTL_FILE.PUT_LINE(file_id_cvs, v_ligne);
          ...
          /*
          if  (S_FDHT_EXP.CTR_KM_PIVOT = 0) then val_ecart_km := null ; -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, ' CTR_KM_PIVOT=0, val_ecart_km = null' );
           elsif (S_FDHT_EXP.CTR_KM_PIVOT is null)  then val_ecart_km := null ; -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'CTR_KM_PIVOT is null, val_ecart_km = null' );
           elsif  (S_FDHT_EXP.GIFP_PROJ_KM_FCT = 0) then val_ecart_km := null ; -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT = 0, val_ecart_km = null' );
           elsif (S_FDHT_EXP.GIFP_PROJ_KM_FCT is null)  then val_ecart_km := null ; -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT is null, val_ecart_km = null' );
             elsif        ( NVL(S_FDHT_EXP.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDHT_EXP.CTR_KM_PIVOT,'0') ) =0  then val_ecart_km := null; --  RES := FPH.FPHQAUT.F_WRITE(FILE_ID, 'diff=0, val_ecart_km = null' );                                                                                                             
          else   val_ecart_km := (S_FDHT_EXP.GIFP_PROJ_KM_FCT - S_FDHT_EXP.CTR_KM_PIVOT) ; -- RES := FPH.FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km = ' ||val_ecart_km);
          end if; 
             */
...
S_FDHT_EXP.PDV_FOURNISSEUR_VILLE  || '|';
          ...
          n_char := length(v_ligne); 
				...
				EXCEPTION
					WHEN OTHERS THEN
						...
						res     := FPH.FPHQAUT.F_WRITE(file_id, 'EXPORT_FPHQT2_ES ## Message Erreur pl/sql : ' || SQLERRM );
            res := FPH.FPHQAUT.F_WRITE(file_id, 'Erreur 0 ## Nombre de lignes ecrites :' || N_SAUV);
             res := FPH.FPHQAUT.F_WRITE(file_id, 'Erreur 0 ## lenght(v_ligne) :' || n_char);
            res := FPH.FPHQAUT.F_WRITE(file_id, 'Erreur 0 ## v_ligne :' || V_LIGNE);
					RETURN V_ERR;
    ...
    EXCEPTION 
			WHEN OTHERS THEN
				...
				V_ERR  := 1;
				res     := FPH.FPHQAUT.F_WRITE(file_id,'EXPORT_FPHQT2_ES ## Message Erreur pl/sql : ' || SQLERRM);
        res := FPH.FPHQAUT.F_WRITE(file_id, 'Erreur 2 ## Nombre de lignes ecrites :' || N_SAUV);
...
From Fph.Fphqt90, Fph.Fphqt91, Fph.Fphqt92, Fph.Fphqtti, Fph.Fphqtvb, Fph.Fphqtde, Fph.Fphqtfv
Where Fph.Fphqt90.Ie_Affaire = Fph.Fphqt91.Ie_Affaire
And Fph.Fphqt92.Ie_Affaire(+) = Fph.Fphqt90.Ie_Affaire
And Fph.Fphqtti.Ie_Affaire(+) = Fph.Fphqt90.Ie_Affaire
And Fph.Fphqtvb.Vb_Vin(+) = Fph.Fphqt90.Vin
And Fph.Fphqt90.Ie_Affaire = FPH.fphqtde.De_Num_Contrat(+)
And Fph.Fphqtde.De_Fg_Version(+) = 1
And Fph.Fphqtfv.Fv_Cd_Fam_Vehicule(+) = Fph.Fphqtde.Fv_Cd_Fam_Vehicule
And Fph.Fphqtfv.Fv_Cdpays(+) = Fph.Fphqtde.Py_Cd_Pays    
And FPH.Fphqt90.Py_Cd_Pays = 'ES' ;  -- AJOUT MARDI
    
    
   BEGIN

				FILE_ID := FPHQAUT.F_OPEN(NOMLOG);
        RES := FPHQAUT.F_WRITE(FILE_ID, '-------------     BEGIN     ------------------');
           ...
           if  (S_FDHT_ALIM.CTR_KM_PIVOT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' CTR_KM_PIVOT=0, val_ecart_km = null' );
           elsif (S_FDHT_ALIM.CTR_KM_PIVOT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'CTR_KM_PIVOT is null, val_ecart_km = null' );
           elsif  (S_FDHT_ALIM.GIFP_PROJ_KM_FCT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT = 0, val_ecart_km = null' );
           elsif (S_FDHT_ALIM.GIFP_PROJ_KM_FCT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT is null, val_ecart_km = null' );
             elsif        ((NVL(S_FDHT_ALIM.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDHT_ALIM.CTR_KM_PIVOT,'0')) =0 ) then val_ecart_km := null; --  RES := FPHQAUT.F_WRITE(FILE_ID, 'diff=0, val_ecart_km = null' );                                                                                                             
          else   val_ecart_km := S_FDHT_ALIM.GIFP_PROJ_KM_FCT - S_FDHT_ALIM.CTR_KM_PIVOT ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km = ' ||val_ecart_km);
          end if; 
          
        --  val_ecart_km := 2 ;
         -- RES := FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km  =  ' ||val_ecart_km);
 /*CASE CTR_KM_PIVOT WHEN 0 THEN  null
                WHEN NULL then null
                else case GIFP_PROJ_KM_FCT when 0 then  null
                                          when null then null
...
PDV_FOURNISSEUR_VILLE  =  S_FDHT_ALIM.PDV_FOURNISSEUR_VILLE 
                ...
                where CTR_NUM_1 = S_FDHT_ALIM.CTR_NUM_1
                and   CTR_NUM_2 = S_FDHT_ALIM.CTR_NUM_2          ----CTR_NUM, VEH_TYP_MINE, VEH_NO_SERIE, VEH_FAM, CD_PAYS 
               and  VEH_TYP_MINE =  S_FDHT_ALIM.VEH_TYP_MINE
                and VEH_NO_SERIE =  S_FDHT_ALIM.VEH_NO_SERIE
              --  and VEH_FAM = S_FDHT_ALIM.VEH_FAM  -------------------------EXCEPTION, AJOUTER APRES QUAND ON AURA LES DONNNES DANS select FV_CD_FAM_VEHICULE from FPHQTFV
                and CD_PAYS ='ES' ;
						...
						WHEN OTHERS THEN  
							...
							V_ERROR := SUBSTR('SQLCODE: '||TO_CHAR(SQLCODE)||' -ERROR: '||SQLERRM,1,4000);   
							RES := FPHQAUT.F_WRITE(FILE_ID, 'ALIM_DONNEES_FDSQT2_ES #WHEN OTHERS THEN# Message Erreur pl/sql :' ||SQLERRM);
...
And Fph.Fphqtfv.Fv_Cd_Fam_Vehicule(+) = Fph.Fphqtde.Fv_Cd_Fam_Vehicule
And Fph.Fphqtfv.Fv_Cdpays(+) = Fph.Fphqtde.Py_Cd_Pays    
And FPH.Fphqt90.Py_Cd_Pays = 'ES' ;  -- AJOUT DU PAYS DEMANDE
		...
		BEGIN
			file_name := p_filename;
			FILE_ID := FPHQAUT.F_OPEN(NOMLOG);
			file_id_cvs:= FPHQAUT.F_OPEN_CVS(p_path, file_name);
			RES     := FPHQAUT.F_WRITE(FILE_ID, 'EXPORT_FDSQT2_ES ## MISE A DISPOSITION du fichier ##');

 			V_LIGNE := 'Date_traitement|CD_PAYS|CD_SOURCE|RESEAU|CTR_NUM|CTR_NUM|BUY_BACK_TYPE|CL_SIREN|CL_NOM|TVA_CD|CL_TYP_LIB|CL_APE_CD|CL_APE_LIB|CL_ADR_FACT|CL_ADR_CP|CL_ADR_VILLE|CTR_RETAIL|CIVILITE_SIGNATAIRE|NOM_SIGNATAIRE|PRENOM_SIGNATAIRE|TEL_CONTACT|EMAIL_SIGNATAIRE|DUREE_INI|KM_PIVOT_INI|ER_PIVOT_INI|CTR_DT_FIN|CTR_DMIN_B2|CTR_DMIN_B1|CTR_DTOT|CTR_DMAX_B1|CTR_DMAX_B2|CTR_KMIN_B2|CTR_KMIN_B1|CTR_KM_PIVOT|CTR_KMAX_B1|CTR_KMAX_B2|CTR_ERMIN_B2|CTR_ERMIN_B1|CTR_ER_HT|CTR_ERMAX_B1|CTR_ERMAX_B2|GIFP_DER_KM|GIFP_DT_DER_KM|GIFP_ORIG_DER_KM|GIFP_PROJ_KM_FCT|v_ecart_km|CTR_TYP_SOUP|REDEV_KM_EXCED|LOY_MTHT_FIN|LOY_MT_HT|LOY_MT_TTC|LOY_FLG_GPF|LOY_FLG_MAIN|LOY_PCT_CEM|LOY_FLG_TR|LOY_FLG_ASS|LOY_FLG_IP|LOY_FLG_AFD|LOY_FLG_CARBU|FLAG_PNEUS_ETE|FLAG_PNEUS_HIVER|FLAG_VEHICULE_DE_REMPLACEMENT|FLAG_CEM_FLEX|FLAG_PNEU_MIXTE|FLAG_JOCKEY|FLAG_PASS_RESTIT|VEH_FAM|VEH_LIB_LS|VEH_LCDV|VEH_LCDV_LIB|CARROSSERIE|VEH_COUL|VEH_GENRE|VEH_NRJ|VEH_CO2|VEH_PUIS_FISC|VEH_TYP_MINE|VEH_NO_SERIE|VEH_BLVD|VEH_NO_IMMAT|CTR_DT_LIV|NOM_CHAUFFEUR|GENRE_CD_TRANSCO|VEH_MTHT_CAT|VEH_MTHT_OPT|VEH_PCT_REM|VEH_MTHT_ACC|VEH_MTHT_TRANS|VEH_GARN|VEH_OPT1|VEH_OPT2|VEH_OPT3|VEH_OPT4|VEH_OPT5|VEH_OPT6|VEH_OPT7|VEH_OPT8|VEH_OPT9|VEH_OPT10|VEH_OPT11|VEH_ACC1|VEH_ACC2|VEH_ACC3|VEH_ACC4|VEH_ACC5|VEH_TRF1|VEH_TRF2|VEH_TRF3|VEH_TRF4|PDV_RP_RRDI|PDV_RP_NOM|PDV_REPRENEUR_RAISON_SOCIALE|PDV_REPRENEUR_ADRESSE|PDV_REPRENEUR_CODE_POSTAL|PDV_REPRENEUR_VILLE|PDV_FT_RRDI|PDV_FT_NOM|CD_VENDEUR|PDV_VD_NOM|PDV_FOURNISSEUR_RAISON_SOCIALE|PDV_FOURNISSEUR_ADRESSE|PDV_FOURNISSEUR_CODE_POSTAL|PDV_FOURNISSEUR_VILLE'; 
 			
			RES := FPHQAUT.F_WRITE_CVS(file_id_cvs, V_LIGNE);
          ...
          if  (S_FDHT_EXP.CTR_KM_PIVOT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' CTR_KM_PIVOT=0, val_ecart_km = null' );
           elsif (S_FDHT_EXP.CTR_KM_PIVOT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'CTR_KM_PIVOT is null, val_ecart_km = null' );
           elsif  (S_FDHT_EXP.GIFP_PROJ_KM_FCT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT = 0, val_ecart_km = null' );
           elsif (S_FDHT_EXP.GIFP_PROJ_KM_FCT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT is null, val_ecart_km = null' );
             elsif        ((NVL(S_FDHT_EXP.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDHT_EXP.CTR_KM_PIVOT,'0')) =0 ) then val_ecart_km := null; --  RES := FPHQAUT.F_WRITE(FILE_ID, 'diff=0, val_ecart_km = null' );                                                                                                             
          else   val_ecart_km := S_FDHT_EXP.GIFP_PROJ_KM_FCT - S_FDHT_EXP.CTR_KM_PIVOT ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km = ' ||val_ecart_km);
          end if; 
...
S_FDHT_EXP.PDV_FOURNISSEUR_VILLE    || '|'   ;
				...
				EXCEPTION
					WHEN OTHERS THEN
						COMMIT;
						V_ERR  := 1;
						res     := FPHQAUT.F_WRITE(file_id, 'EXPORT_FDSQT2_ES ## Message Erreur pl/sql : ' || SQLERRM );
					RETURN V_ERR;
			...
			END LOOP;
    			...
    			UTL_FILE.FCLOSE(file_id_cvs);
			res := FPHQAUT.F_WRITE(file_id, 'EXPORT_FDSQT2_ES ## Nombre de lignes ecrites :' || N_SAUV);
			UTL_FILE.FCLOSE(file_id);
...
PACKAGE BODY Fcreate or replace
PACKAGE BODY FPHQACT2 AScreate or replace
PACKAGE BODY FPHQACT2 AS
...
And FPH.Fphqt90.Py_Cd_Pays = 'ES' ;  -- AJOUT MARDI
   ...
   BEGIN
        ...
        RES := FPHQAUT.F_WRITE(FILE_ID, '-------------     BEGIN     ------------------');
           ...
           if  (S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' CTR_KM_PIVOT=0, val_ecart_km = null' );
           elsif (S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'CTR_KM_PIVOT is null, val_ecart_km = null' );
           elsif  (S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT = 0, val_ecart_km = null' );
           elsif (S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT is null, val_ecart_km = null' );
             elsif        ((NVL(S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT,'0')) =0 ) then val_ecart_km := null; --  RES := FPHQAUT.F_WRITE(FILE_ID, 'diff=0, val_ecart_km = null' );                                                                                                             
          else   val_ecart_km := S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT - S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km = ' ||val_ecart_km);
          end if; 
...
VEH_FAM  =  S_FDSQTT_TESTFORMAT_C.VEH_FAM ,  ---------EXCEPTION, SUPPRIMER APRES QUAND ON AURA LES DONNNES DANS select FV_CD_FAM_VEHICULE from FPHQTFV
...
PDV_FOURNISSEUR_VILLE  =  S_FDSQTT_TESTFORMAT_C.PDV_FOURNISSEUR_VILLE 
              ...
              --  and VEH_FAM = S_FDSQTT_TESTFORMAT_C.VEH_FAM  -------------------------EXCEPTION, AJOUTER APRES QUAND ON AURA LES DONNNES DANS select FV_CD_FAM_VEHICULE from FPHQTFV
                and CD_PAYS ='ES' ;
...
And FPH.Fphqt90.Py_Cd_Pays = 'ES' ;  -- AJOUT MARDI
		...
		BEGIN
			...
			FOR S_FDSQT_TESTFORMAT_C IN C_FDSQTT_TESTFORMAT_C LOOP 
          ...
          if  (S_FDSQT_TESTFORMAT_C.CTR_KM_PIVOT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' CTR_KM_PIVOT=0, val_ecart_km = null' );
           elsif (S_FDSQT_TESTFORMAT_C.CTR_KM_PIVOT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'CTR_KM_PIVOT is null, val_ecart_km = null' );
           elsif  (S_FDSQT_TESTFORMAT_C.GIFP_PROJ_KM_FCT = 0) then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT = 0, val_ecart_km = null' );
           elsif (S_FDSQT_TESTFORMAT_C.GIFP_PROJ_KM_FCT is null)  then val_ecart_km := null ; -- RES := FPHQAUT.F_WRITE(FILE_ID, 'GIFP_PROJ_KM_FCT is null, val_ecart_km = null' );
             elsif        ((NVL(S_FDSQT_TESTFORMAT_C.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDSQT_TESTFORMAT_C.CTR_KM_PIVOT,'0')) =0 ) then val_ecart_km := null; --  RES := FPHQAUT.F_WRITE(FILE_ID, 'diff=0, val_ecart_km = null' );                                                                                                             
          else   val_ecart_km := S_FDSQT_TESTFORMAT_C.GIFP_PROJ_KM_FCT - S_FDSQT_TESTFORMAT_C.CTR_KM_PIVOT ; -- RES := FPHQAUT.F_WRITE(FILE_ID, ' val_ecart_km = ' ||val_ecart_km);
          end if; 
          ...
          V_LIGNE := 
          S_FDSQT_TESTFORMAT_C.Date_traitement    || '|' ||
  S_FDSQT_TESTFORMAT_C.CD_PAYS    || '|' ||
S_FDSQT_TESTFORMAT_C.CD_SOURCE    || '|' ||
S_FDSQT_TESTFORMAT_C.RESEAU    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_NUM_1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_NUM_2    || '|' ||
S_FDSQT_TESTFORMAT_C.BUY_BACK_TYPE    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_SIREN    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_NOM    || '|' ||
S_FDSQT_TESTFORMAT_C.TVA_CD    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_TYP_LIB    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_APE_CD    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_APE_LIB    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_ADR_FACT    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_ADR_CP    || '|' ||
S_FDSQT_TESTFORMAT_C.CL_ADR_VILLE    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_RETAIL    || '|' ||
S_FDSQT_TESTFORMAT_C.CIVILITE_SIGNATAIRE    || '|' ||
S_FDSQT_TESTFORMAT_C.NOM_SIGNATAIRE    || '|' ||
S_FDSQT_TESTFORMAT_C.PRENOM_SIGNATAIRE    || '|' ||
S_FDSQT_TESTFORMAT_C.TEL_CONTACT    || '|' ||
S_FDSQT_TESTFORMAT_C.EMAIL_SIGNATAIRE    || '|' ||
S_FDSQT_TESTFORMAT_C.DUREE_INI    || '|' ||
S_FDSQT_TESTFORMAT_C.KM_PIVOT_INI    || '|' ||
S_FDSQT_TESTFORMAT_C.ER_PIVOT_INI    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DT_FIN    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DMIN_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DMIN_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DTOT    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DMAX_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DMAX_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_KMIN_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_KMIN_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_KM_PIVOT    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_KMAX_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_KMAX_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_ERMIN_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_ERMIN_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_ER_HT    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_ERMAX_B1    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_ERMAX_B2    || '|' ||
S_FDSQT_TESTFORMAT_C.GIFP_DER_KM    || '|' ||
S_FDSQT_TESTFORMAT_C.GIFP_DT_DER_KM    || '|' ||
S_FDSQT_TESTFORMAT_C.GIFP_ORIG_DER_KM    || '|' ||
S_FDSQT_TESTFORMAT_C.GIFP_PROJ_KM_FCT    || '|' ||
val_ecart_km    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_TYP_SOUP    || '|' ||
S_FDSQT_TESTFORMAT_C.REDEV_KM_EXCED    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_MTHT_FIN    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_MT_HT    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_MT_TTC    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_GPF    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_MAIN    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_PCT_CEM    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_TR    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_ASS    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_IP    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_AFD    || '|' ||
S_FDSQT_TESTFORMAT_C.LOY_FLG_CARBU    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_PNEUS_ETE    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_PNEUS_HIVER    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_VEHICULE_DE_REMPLACEMENT    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_CEM_FLEX    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_PNEU_MIXTE    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_JOCKEY    || '|' ||
S_FDSQT_TESTFORMAT_C.FLAG_PASS_RESTIT    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_FAM    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_LIB_LS    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_LCDV    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_LCDV_LIB    || '|' ||
S_FDSQT_TESTFORMAT_C.CARROSSERIE    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_COUL    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_GENRE    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_NRJ    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_CO2    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_PUIS_FISC    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_TYP_MINE    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_NO_SERIE    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_BLVD    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_NO_IMMAT    || '|' ||
S_FDSQT_TESTFORMAT_C.CTR_DT_LIV    || '|' ||
S_FDSQT_TESTFORMAT_C.NOM_CHAUFFEUR    || '|' ||
S_FDSQT_TESTFORMAT_C.GENRE_CD_TRANSCO    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_MTHT_CAT    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_MTHT_OPT    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_PCT_REM    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_MTHT_ACC    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_MTHT_TRANS    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_GARN    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT1    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT2    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT3    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT4    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT5    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT6    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT7    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT8    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT9    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT10    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_OPT11    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_ACC1    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_ACC2    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_ACC3    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_ACC4    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_ACC5    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_TRF1    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_TRF2    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_TRF3    || '|' ||
S_FDSQT_TESTFORMAT_C.VEH_TRF4    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_RP_RRDI    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_RP_NOM    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_REPRENEUR_RAISON_SOCIALE    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_REPRENEUR_ADRESSE    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_REPRENEUR_CODE_POSTAL    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_REPRENEUR_VILLE    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FT_RRDI    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FT_NOM    || '|' ||
S_FDSQT_TESTFORMAT_C.CD_VENDEUR    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_VD_NOM    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FOURNISSEUR_RAISON_SOCIALE    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FOURNISSEUR_ADRESSE    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FOURNISSEUR_CODE_POSTAL    || '|' ||
S_FDSQT_TESTFORMAT_C.PDV_FOURNISSEUR_VILLE    || '|'   ;
...
END FPHQACT2;PHQACT2 AS

  FUNCTION ALIM_DONNEES_FDSQT2_ES (NOMLOG      VARCHAR2,
                                  P_DATE_TRAI   DATE,
                                  V_INSERTS     OUTcreate or replace
PACKAGE BODY FPHQACT2 AS
...
AND FPHQTFV.FV_CDPAYS(+) = FPHQTDE.PY_CD_PAYS;   -- AVANT / AND FPHQTFV.FC_CD_PAYS(+) = FPHQTDE.PY_CD_PAYS;
   ...
   BEGIN
         ...
         /*  if  (S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT = 0) then v_ecart_km := null ;
           elsif (S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT = null)  then v_ecart_km := null ;
           elsif  (S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT = 0) then v_ecart_km := null ;
           elsif (S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT = null)  then v_ecart_km := null ;
             elsif        ((NVL(S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT,'0') - NVL(S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT,'0')) =0 ) then v_ecart_km := null;
          else   v_ecart_km := S_FDSQTT_TESTFORMAT_C.GIFP_PROJ_KM_FCT - S_FDSQTT_TESTFORMAT_C.CTR_KM_PIVOT ;
          end if; 
          RES := FPHQAUT.F_WRITE(FILE_ID, ' test : ' ||v_ecart_km);
          */
...
S_FDSQTT_TESTFORMAT_C.VEH_CO2 ,
S_FDSQTT_TESTFORMAT_C.VEH_PUIS_FISC ,
S_FDSQTT_TESTFORMAT_C.VEH_TYP_MINE ,create or replace
PACKAGE BODY FPHQACT2 AS
